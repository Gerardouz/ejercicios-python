...
class Arbolb:
	...
	def faltantes (self, raiz = None):
		...
		if (raiz.hizq != None):
			M = self.__mayor
			cons = self.faltantes (raiz.hizq)
			if ((self.__mayor + 1) != raiz.info):
				self.__faltantes.append(self.__mayor + 1)
			self.__mayor = M
		
		if (raiz.hder != None):
			m = self.__menor 
			cons = self.faltantes (raiz.hder)
			if ( raiz.info != (self.__menor - 1)):
				self.__faltantes.append(self.__menor - 1)
			self.__menor = m
	...
	# FORMA 3: la funcion recursiva n_nodos() crea un atributo al arbol self.__n donde sumara los valores
	...
	def alturaizq(self, raiz = None):
		...
		if (raiz.hizq != None):
			...
			self.alturaizq(raiz.hizq)

			
		return self.__altura

	def alturader (self,raiz = None):
	...
	def balanceado(self, raiz = None):
		...
		altura = self.alturaizq() - self.alturader()
		
		if (altura >= -1 and altura <= 1):
